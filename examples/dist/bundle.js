...
var VirtualList = React.createClass({ displayName: "VirtualList",
    ...
    render: function () {
        return React.createElement("div", React.__spread({}, this.props, { style: { boxSizing: 'border-box', height: this.state.height, paddingTop: this.state.bufferStart } }), this.state.items.map(this.props.renderItem));
    }
});
...
var _reactVirtualList = require('react-virtual-list');
...
var Select = _react2['default'].createClass({
	...
	componentDidUpdate: function componentDidUpdate(prevProps, prevState) {
		...
		var shouldRenderList = this.state.isOpen && this.refs.list && this.refs.list.props.container === window;
		if (shouldRenderList) {
			this.setState({
	...
	filterOptions: function filterOptions(excludeOptions) {
		...
		for (var i = 0; i < options.length; i++) {
			...
			var _name = option.label.toLowerCase().trim();
			var isValid = true;

			if (_name.indexOf(query) !== 0) {
			...
			if (isValid) {
				if (!matchResults[_name.length]) {
					matchResults[_name.length] = [];
				}
				matchResults[_name.length].push(option);
			}
		}
...
},{"./Async":4,"./Option":5,"./Value":6,"./utils/stripDiacritics":7,"classnames":undefined,"lodash":1,"react":undefined,"react-dom":undefined,"react-input-autosize":undefined,"react-virtual-list":2}]},{},[]);
